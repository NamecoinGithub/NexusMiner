...
public:
	...
	// Read config file - automatically detects format (.config = TOML, .conf = JSON)
	bool read_config(std::string const& miner_config_file);
	
	// Read TOML format config file
	bool read_toml_config(std::string const& miner_config_file);
	...
	void set_logfile(const std::string& file) { m_logfile = file; }
	...
	void set_miner_falcon_pubkey(const std::string& key) { m_miner_falcon_pubkey = key; }
	void set_miner_falcon_privkey(const std::string& key) { m_miner_falcon_privkey = key; }
...
private:
	...
	bool read_json_config(std::string const& miner_config_file);
