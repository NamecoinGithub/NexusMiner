    ...
    std::function<void()> recovery_initiated_fn)
{
    ...
    if (template_interface && template_interface->has_valid_template())
    {
        ...
        // the current mining template.  Workers keep mining and submitting on
        // the current template. A fresh template is requested opportunistically
        // so that the hashPrevBlock can be updated for the next submission cycle.
