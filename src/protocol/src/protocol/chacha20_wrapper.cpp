    ...
    const std::vector<uint8_t>& session_key)
{
    ...
    try {
        nonce = generate_nonce(); // 12 cryptographically-random bytes
    } catch (const std::exception& e) {
        result.error_message = std::string("Nonce generation failed: ") + e.what();
        m_logger->error("[ChaCha20 SubmitBlock] {}", result.error_message);
        return result;
    }

    // ── Encrypt with ChaCha20-Poly1305 (empty AAD for SUBMIT_BLOCK) ────────
    // The node-side LLC::DecryptPayloadChaCha20() passes no AAD, so we must
    // also use an empty AAD here for the authentication tag to verify.
    static const std::vector<uint8_t> EMPTY_AAD{};
    auto enc = encrypt(plaintext, session_key, nonce, EMPTY_AAD);
    if (!enc.success || enc.data.empty()) {
        result.error_message = "ChaCha20 encryption failed: " + enc.error_message;
        m_logger->error("[ChaCha20 SubmitBlock] {}", result.error_message);
        return result;
    }

    // ── Assemble wire payload: [nonce(12)][ciphertext][tag(16)] ───────────
    result.data.reserve(CHACHA20_NONCE_SIZE + enc.data.size());
