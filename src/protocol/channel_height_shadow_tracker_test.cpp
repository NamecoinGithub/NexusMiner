 ...
 *  2.  IngestBlockData updates canonical; no shadow/push/health spillover
 *  3.  IngestBlockData is monotonic — stale BLOCK_DATA cannot regress canonical
 *  4.  IngestKeepaliveAck updates shadow only; no canonical spillover
 *  5.  IngestSessionStatusAck updates health only; no canonical/shadow spillover
 *  6.  IngestPushNotification updates push trend only; no canonical/shadow spillover
 *  7.  CrossCheck: canonical agrees with shadow when heights match
 *  8.  CrossCheck: disagreement detected when heights diverge by >1
 ...
 * 21.  most_recent_source tracks the last ingested source correctly
 ...
 * 24.  CrossCheck::is_disagreement() respects tolerance parameter
...
void test_ingest_block_data_canonical_only() {
    std::cout << "\nTest 2: IngestBlockData updates canonical; no shadow/push/health spillover\n";
    ChannelHeightShadowTracker t;
...
void test_ingest_keepalive_ack_shadow_only() {
    std::cout << "\nTest 4: IngestKeepaliveAck updates shadow only; no canonical spillover\n";
    ChannelHeightShadowTracker t;
...
void test_ingest_session_status_health_only() {
    std::cout << "\nTest 5: IngestSessionStatusAck updates health only; no canonical/shadow spillover\n";
    ChannelHeightShadowTracker t;
    ...
    // Confirm no spillover to canonical, shadow, or push
    print_test_result("canonical.initialized == false",          !snap.canonical.is_initialized());
    print_test_result("shadow.initialized == false",             !snap.shadow.is_initialized());
    print_test_result("push_trend.initialized == false",         !snap.push_trend.is_initialized());
    print_test_result("canonical.unified_height == 0",           snap.canonical.unified_height == 0);
...
void test_ingest_push_trend_only() {
    std::cout << "\nTest 6: IngestPushNotification updates push trend only; no canonical/shadow spillover\n";
    ChannelHeightShadowTracker t;
...
void test_cross_check_disagreement_detected() {
    std::cout << "\nTest 8: CrossCheck: disagreement detected when heights diverge by >1\n";
    ChannelHeightShadowTracker t;
    t.IngestBlockData(5005, 200);
    t.IngestKeepaliveAck(5000, 200, 300, 50, 0);  // shadow 5 blocks behind canonical
    auto r = t.CrossCheck();
...
void test_get_snapshot_all_layers() {
    std::cout << "\nTest 14: GetSnapshot returns all four layers consistently\n";
    ChannelHeightShadowTracker t;
    t.IngestBlockData(5000, 200);
    t.IngestKeepaliveAck(5000, 200, 300, 50, 0);
...
void test_cross_check_tolerance() {
    std::cout << "\nTest 24: CrossCheck::is_disagreement() respects tolerance parameter\n";
    ChannelHeightShadowTracker t;
    t.IngestBlockData(5003, 200);
    t.IngestKeepaliveAck(5000, 200, 300, 50, 0);  // delta = 3
    auto r = t.CrossCheck();
