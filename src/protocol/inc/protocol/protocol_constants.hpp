...
namespace ProtocolConstants {
    ...
    //   * protocol/falcon_constants.hpp  -> derives PRIME_VOFFSETS_MAX_SIZE
    //                                        and the SUBMIT_BLOCK_WRAPPER_*_MAX
    //                                        wire-format upper bounds.
    //   * cpu/inc/cpu/prime_validation.hpp -> re-exports under
    //                                          `nexusminer::prime::` and
    //                                          implements is_well_formed_prime_offsets().
    //
    // Living in this leaf header (only depends on <cstddef>/<cstdint> and the
    // leaf mining/mining_constants.hpp) means the cross-cutting drift checks
    // below fire on every translation unit that touches either header — they
    // are NOT gated on WITH_PRIME=ON build flags.
    //
    // Bounds rationale:
    ...
    constexpr std::size_t kPrimeOffsetFractionBytes = 4;
    ...
    constexpr std::size_t kMaxRecognisedChainLength = 19;

    /** Minimum well-formed serialized vOffsets size (chain length 2). */
    constexpr std::size_t kMinSerializedPrimeOffsets =
    ...
    constexpr std::size_t kMaxSerializedPrimeOffsets =
    ...
    // without also bumping the layout constants.  Lives here (not in the prime
    // validation header) so the assert fires on every TU that includes this
    // header — including builds with WITH_PRIME=OFF.
    static_assert(
        ...
        "kMaxRecognisedChainLength in protocol_constants.hpp");
    static_assert(
