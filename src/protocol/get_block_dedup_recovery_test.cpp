 ...
 * 17. GetBlockReason dedup bypass policy validation (bypass_all includes HEALTH_NO_TEMPLATE)
 * 18. Rapid-burst guard applies to non-bypassing reasons; bypassing reasons exempt
 * 19. Pending in-flight suppression vs. forced reasons — bypass_all reasons exempt from in-flight guard
...
// ============================================================================
// Test 18: Rapid-burst guard applies before reason bypass policy
// ============================================================================
// Non-bypassing reasons (INITIAL_REQUEST, TEMPLATE_AGE_WARNING, etc.) are
// suppressed by the rapid-burst guard immediately after a GET_BLOCK is sent.
// Bypass_all reasons (RECOVERY_FORCED, RECOVERY_TIMER, BLOCK_ACCEPTED,
// HEALTH_NO_TEMPLATE) are EXEMPT from the rapid-burst guard and always allowed
// through (handled at a higher level; they skip the burst check in Solo).
void test_rapid_burst_guard_applies_to_non_bypassing_reasons() {
    std::cout << "\nTest 18: Rapid-burst guard applies to non-bypassing reasons\n";
    ...
    // Immediately after — non-bypassing reason sees burst suppression.
    auto age_warning = guard.check(GetBlockReason::TEMPLATE_AGE_WARNING, 101, false, hash_prev);
    // bypass_all reasons bypass the rapid-burst guard entirely.
    ...
    guard.reset();
    // After template-state reset the rapid-burst guard remains (timestamp preserved).
    ...
    print_test_result("TEMPLATE_AGE_WARNING is rapid-burst-suppressed immediately after send",
        age_warning == GetBlockDedupGuard::Verdict::SUPPRESS_RAPID_BURST);
    print_test_result("RECOVERY_FORCED bypasses rapid-burst guard (bypass_all reason)",
        forced == GetBlockDedupGuard::Verdict::ALLOW);
    print_test_result("HEALTH_NO_TEMPLATE bypasses rapid-burst guard (bypass_all reason)",
        health_no_template == GetBlockDedupGuard::Verdict::ALLOW);
    print_test_result("Template-state reset preserves rapid-burst timestamp",
        block_accepted_after_reset == GetBlockDedupGuard::Verdict::ALLOW);
}

// ============================================================================
// Test 19: Pending in-flight guard: bypass_all reasons are exempt; others respect it
//
// Solo::get_work() skips PendingGetBlock::is_pending_for() when
// should_bypass_all_dedup(reason)==true.  This ensures degraded-mode retries
// (RECOVERY_FORCED, RECOVERY_TIMER, BLOCK_ACCEPTED, HEALTH_NO_TEMPLATE) are
// never permanently stalled by a stale in-flight record.
// Non-bypassing reasons (PUSH_TIP_MOVED, TEMPLATE_AGE_WARNING, etc.) are still
// suppressed by the pending in-flight guard until it auto-expires (4s).
// ============================================================================
void test_pending_inflight_guard_policy() {
    std::cout << "\nTest 19: Pending in-flight guard — bypass_all exempt, others suppressed\n";
    ...
    // bypass_all reasons skip the pending guard entirely (simulated by caller logic).
    print_test_result("RECOVERY_FORCED bypasses pending guard (should_bypass_all_dedup=true)",
        should_bypass_all_dedup(GetBlockReason::RECOVERY_FORCED));
    print_test_result("RECOVERY_TIMER bypasses pending guard (should_bypass_all_dedup=true)",
        should_bypass_all_dedup(GetBlockReason::RECOVERY_TIMER));
    print_test_result("BLOCK_ACCEPTED bypasses pending guard (should_bypass_all_dedup=true)",
        should_bypass_all_dedup(GetBlockReason::BLOCK_ACCEPTED));
    print_test_result("HEALTH_NO_TEMPLATE bypasses pending guard (should_bypass_all_dedup=true)",
        should_bypass_all_dedup(GetBlockReason::HEALTH_NO_TEMPLATE));

    // Non-bypassing reasons are suppressed by a healthy pending request.
    bool non_bypass_suppressed = !should_bypass_all_dedup(GetBlockReason::PUSH_TIP_MOVED) &&
                                  pending.is_pending_for(100);
    print_test_result("PUSH_TIP_MOVED suppressed by healthy pending in-flight",
        non_bypass_suppressed);
    bool age_suppressed = !should_bypass_all_dedup(GetBlockReason::TEMPLATE_AGE_WARNING) &&
                           pending.is_pending_for(100);
    print_test_result("TEMPLATE_AGE_WARNING suppressed by healthy pending in-flight",
        age_suppressed);

    // After the 4-second timeout the pending guard auto-expires.
    pending.sent_at -= std::chrono::seconds(5);  // expire by 1 second
    bool expired_suppressed = !should_bypass_all_dedup(GetBlockReason::PUSH_TIP_MOVED) &&
                               pending.is_pending_for(100);
    print_test_result("Non-bypass reason allowed after pending timeout (4s)",
        !expired_suppressed);

    // Lower pending height does not suppress higher-height request.
    pending.sent_at        = std::chrono::steady_clock::now();
    pending.unified_height = 99;
    bool lower_height_suppressed = !should_bypass_all_dedup(GetBlockReason::PUSH_TIP_MOVED) &&
                                    pending.is_pending_for(100);
    print_test_result("Pending at height 99 does NOT suppress request for height 100",
        !lower_height_suppressed);
}
...
void test_should_bypass_height_state_after_cooldown_alias() {
    ...
    print_test_result("RECOVERY_FORCED: both helpers return true",
        should_bypass_height_state_after_cooldown(GetBlockReason::RECOVERY_FORCED));
    print_test_result("HEALTH_NO_TEMPLATE: both helpers return true (bypass_all)",
        should_bypass_height_state_after_cooldown(GetBlockReason::HEALTH_NO_TEMPLATE));
    print_test_result("INITIAL_REQUEST: both helpers return false",
        !should_bypass_height_state_after_cooldown(GetBlockReason::INITIAL_REQUEST));
    print_test_result("TEMPLATE_AGE_WARNING: both helpers return false",
        !should_bypass_height_state_after_cooldown(GetBlockReason::TEMPLATE_AGE_WARNING));
}
...
int main() {
    ...
    test_rapid_burst_guard_applies_to_non_bypassing_reasons();
    test_pending_inflight_guard_policy();
