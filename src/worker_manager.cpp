    ...
    m_recovery_sm.set_cancel_forced_retry_callback([this]() {
        ...
        m_primary_node_session->set_template_handler(
            [weak_self = weak_from_this()](const ::LLP::CBlock& block, uint32_t nBits) {
                ...
                TemplateDistributor::on_template_received(block, nBits, self);
            }
        );
        ...
        m_primary_node_session->set_session_authenticated_handler(
            [weak_self = weak_from_this()](uint32_t session_id) {
                ...
                if (self->m_sim_link.is_using_failover())
                {
...
void Worker_manager::retry_connect(network::Endpoint const& wallet_endpoint)
{
    ...
    // ── Failover switchover logic (delegated to DualConnectionManager) ────────
    network::Endpoint effective_endpoint = wallet_endpoint;
    bool failover_switched = false;

    if (m_config.has_failover())
    {
        auto decision = m_sim_link.record_connection_failure();
        failover_switched = decision.switched;

        if (decision.switched) {
            m_connection_backoff.reset();  // reset backoff for the new node
            if (m_sim_link.is_using_failover()) {
                m_logger->warn("[Failover] Primary {} failed {} times — switching to failover {}",
                    m_primary_endpoint.to_string(),
                    m_sim_link.failover_max_retries(),
        ...
        } else if (m_sim_link.is_using_failover()) {
            effective_endpoint = m_failover_endpoint;
        }
...
Worker_manager::FailoverStatus Worker_manager::get_failover_status() const
{
    ...
    fs.using_failover          = m_sim_link.is_using_failover();
    fs.primary_fail_count      = m_sim_link.primary_fail_count();
    fs.failover_max_retries    = m_sim_link.failover_max_retries();
    ...
    fs.failover_activated_at   = m_sim_link.failover_activated_at();
    return fs;
}
...
bool Worker_manager::connect(network::Endpoint const& wallet_endpoint)
{
    ...
    if (!m_primary_endpoint.is_valid())
    {
        ...
        if (m_config.has_failover())
        {
            ...
            m_sim_link.init_failover(m_config.get_failover_max_retries());
    ...
    return m_primary_node_session->connect(wallet_endpoint, [weak_self, wallet_endpoint](bool success) {
        ...
        self->m_sim_link.reset_fail_count();
...
void Worker_manager::retry_template_request(bool bForce)
{
    ...
    if (work_payload && !work_payload->empty()) {
        ...
        m_logger->info("[Worker_manager] → GET_BLOCK sent (recovery epoch {})", m_coordinator->recovery_epoch());
    } else {
    ...
    if (template_age > protocol::ProtocolConstants::TEMPLATE_AGE_EMERGENCY_TIMEOUT_SECONDS) {
        ...
        if (is_recovery_active()) {
            m_logger->debug("[Worker_manager] Emergency aging ({}s) suppressed during active recovery (epoch {})",
                            template_age, m_coordinator->recovery_epoch());
