...
struct RecoveryContext {
    // Atomic: read by timer/handler callbacks on the io_context strand and
    // written by enter_terminal_degraded_mode() which may be called from the
    // signal-handler thread.  Relaxed ordering is sufficient — callers only
    // need a consistent snapshot, not happens-before with other fields.
