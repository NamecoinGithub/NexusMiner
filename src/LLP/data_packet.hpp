 ...
 * a Falcon signature for verification. The signature is verified by nodes but NOT
 * stored permanently, allowing blockchain size reduction.
 * 
 ...
 * is the authenticated miner, without storing the signature permanently.
 */
struct DataPacket
{
    ...
    /** Falcon signature over (merkle_root + nonce) - discarded after verification */
     ...
     * @param sig The Falcon signature (temporary, for verification only)
     */
    ...
    std::vector<uint8_t> serialize() const
    {
        ...
        // 4. Signature bytes (external wrapper - discarded after verification)
    ...
    static DataPacket deserialize(std::vector<uint8_t> const& data)
    {
        ...
        // 6. Extract signature bytes (external wrapper - to be discarded after verification)
    ...
    /**
     * @brief Extract the block data portion (merkle_root + nonce) for blockchain storage
     * 
     * This method extracts only the permanent block data, excluding the signature.
     * The signature is only used for verification and should be discarded.
     * 
     * @return 72-byte vector containing merkle_root (64 bytes) + nonce (8 bytes, big-endian)
     */
    std::vector<uint8_t> get_block_data() const
    {
        std::vector<uint8_t> block_data;
        block_data.reserve(DATA_TO_SIGN_SIZE);
